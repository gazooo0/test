...
st.markdown("""
<div style='line-height: 1.2; font-size: 0.9em; color: white;'>
...
st.markdown("<b>基本的には「利用する」を選択してください。過去に誰かが1回でも検索していればすぐ結果を表示します。</b>")

st.markdown("<b>下記のタイミングの時は古い情報を参照する可能性があるため「最新情報を取得する」を選択してください。</b><br>■特別登録：前週日曜日の18時前後（G1レースは前々週に特別登録となりますが対応していません）<br>■出走想定：水曜日の20時前後<br>■出走確定：木曜日の19時前後<br>■枠順確定：レース前日の11時前後")
