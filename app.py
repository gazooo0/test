...
use_cache = st.radio("キャッシュを", ["利用する", "最新情報を取得する"], horizontal=True)
st.markdown("#### 基本的には「利用する」を選択してください。過去に誰かが1回でも検索していればすぐ結果を表示します。")
st.markdown("#### 下記のタイミングの時は古い情報を参照する可能性があるため「最新情報を取得する」を選択してください。")
